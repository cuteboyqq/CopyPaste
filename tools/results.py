...
from ultralytics.yolo.utils.plotting import Annotator, colors, save_one_box, cls_to_color
...
class Results(SimpleClass):
    ...
    def plot(
        ...
        img_file = self.path.split(os.sep)[-1]
        save_drivable_mask_dir = "/home/ali/Projects/datasets/nuimages/drivable/mask"
        save_drivable_colomap_dir = "/home/ali/Projects/datasets/nuimages/drivable/colormap"
        ## drivable mask
        os.makedirs(save_drivable_mask_dir,exist_ok=True)
        save_dri_mask_path = os.path.join(save_drivable_mask_dir,img_file)

        os.makedirs(save_drivable_colomap_dir,exist_ok=True)
        save_dri_colormap_path = os.path.join(save_drivable_colomap_dir,img_file)


        save_lane_mask_dir = "/home/ali/Projects/datasets/nuimages/lane/mask"
        save_lane_colomap_dir = "/home/ali/Projects/datasets/nuimages/lane/colormap"

        ## drivable mask
        os.makedirs(save_lane_mask_dir,exist_ok=True)
        save_lane_mask_path = os.path.join(save_lane_mask_dir,img_file)

        os.makedirs(save_lane_colomap_dir,exist_ok=True)
        save_lane_colormap_path = os.path.join(save_lane_colomap_dir,img_file)
        ...
        if self.drive_map is not None:
            ...
            cv2.imwrite(save_dri_mask_path,self.drive_map)

            self.drive_map = cls_to_color(self.drive_map, 'drive')
            cv2.imwrite(save_dri_colormap_path,self.drive_map)
        ...
        if self.lane_map is not None:
            ...
            cv2.imwrite(save_lane_mask_path,self.lane_map)

            self.lane_map = cls_to_color(self.lane_map, 'lane')
            cv2.imwrite(save_lane_colormap_path,self.lane_map)
